...
class UpscanController @Inject() (
    ...
    upscanCsvFilesList flatMap { fileList =>
      if (fileList.noOfFilesToUpload == fileList.noOfUploads) {
        fetchCsvCallbackList(fileList).withRetry(appConfig.allCsvFilesCacheRetryAmount) { list =>
          ...
          logger.info(
            s"[UpscanController][successCsv] Comparing cached files [${list.size}] to numberOfFileToUpload[${fileList.noOfFilesToUpload}]"
          )
          logger.info(
            s"[UpscanController][successCsv] Checking if all files have completed upload - [${list.forall(_.isComplete)}]"
          )
          ...
          sessionCacheService.cache[UpscanCsvFilesCallbackList](ersUtil.CALLBACK_DATA_KEY_CSV, callbackData).map { ele =>
            if (callbackData.areAllFilesSuccessful()) {
              ...
              logger.error(
                s"[UpscanController][successCsv] Not all files are completed uploading - (${callbackData.areAllFilesComplete()}) " +
          ...
          logger.error(
            s"[UpscanController][successCsv] failed to fetch callback data with exception ${e.getMessage}," +
        ...
        logger.info(
          s"[UpscanController][successCsv] - checking the number of files upload to checking service, fileList: $fileList"
        )
  ...
  def successOds(scheme: String): Action[AnyContent] = authAction.async { implicit request =>
    val futureCallbackData: Future[Option[UploadStatus]] =
      sessionCacheService.fetch[UploadStatus](ersUtil.CALLBACK_DATA_KEY).withRetry(appConfig.odsSuccessRetryAmount) {
        _.fold(true) {
    ...
    futureCallbackData flatMap {
      ...
      case e: LoopException[Option[UploadStatus] @unchecked] =>
        logger.error(
          s"[UpscanController][successOds] Failed to verify upload. Upload status: ${e.finalFutureData.flatten}",
          e
        )
        getGlobalErrorPage
      case e: Exception                                      =>
