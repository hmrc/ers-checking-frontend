...
@Singleton
class CheckingServiceController @Inject() (
  ...
  def schemeTypeSelected(): Action[AnyContent] = authAction.async { implicit request =>
    showSchemeTypeSelected(request)
  }

  def showSchemeTypeSelected(implicit request: Request[AnyContent]): Future[Result] =
    CSformMappings.schemeTypeForm
      .bindFromRequest()
      .fold(
        formWithErrors => Future.successful(BadRequest(scheme_type(formWithErrors))),
        formData =>
          sessionCacheService
            .cache[String](ersUtil.SCHEME_CACHE, formData.getSchemeType)
            .map { _ =>
              Redirect(routes.CheckingServiceController.checkFileTypePage())
            }
            .recover { case e: Exception =>
              logger.error(
                "[CheckingServiceController][showSchemeTypeSelected] Unable to save scheme. Error: " + e.getMessage
              )
              getGlobalErrorPage
            }
      )

  def checkFileTypePage(form: Form[CS_checkFileType] = CSformMappings.checkFileTypeForm): Action[AnyContent] =
  ...
  def checkFileTypeSelected(): Action[AnyContent] = authAction.async { implicit request =>
    showCheckFileTypeSelected(request)
  }

  def showCheckFileTypeSelected(implicit request: Request[AnyContent]): Future[Result] =
    CSformMappings.checkFileTypeForm
      .bindFromRequest()
      .fold(
        formWithErrors => Future.successful(BadRequest(check_file_type(formWithErrors))),
        formData =>
          sessionCacheService
            .cache[String](ersUtil.FILE_TYPE_CACHE, formData.getFileType)
            .map { _ =>
            ...
            .recover { case e: Exception =>
              logger.error(
                "[CheckingServiceController][showCheckFileTypeSelected] Unable to save file type. Error: " + e.getMessage
              )
              getGlobalErrorPage
            }
      )

  def checkCsvFilePage(): Action[AnyContent] = authAction.async { implicit request =>
  ...
  def showCheckOdsFilePage()(implicit request: Request[AnyRef], hc: HeaderCarrier, messages: Messages): Future[Result] =
    ...
    } yield Ok(check_file(scheme)(request, messages, upscanResponse, appConfig))) recover { case e: Exception =>
  ...
  def showCheckingSuccessPage()(implicit
    request: RequestWithOptionalEmpRefAndPAYE[AnyContent],
    messages: Messages
  ): Future[Result] =
    Future.successful(Ok(checking_success(request, messages, appConfig)))

  def checkingInvalidFilePage(): Action[AnyContent] = authAction.async { implicit request =>
    showInvalidFilePage()
  }
  ...
  def formatErrorsPage(): Action[AnyContent] = authAction.async { implicit request =>
  ...
  def showFormatErrorsPage()(implicit request: Request[AnyRef]): Future[Result] = {
    ...
    future recover { case e: Exception =>
      logger.error(
        "[CheckingServiceController][showFormatErrorsPage] Unable to fetch file type. Error: " + e.getMessage
      )
      getGlobalErrorPage
    }
  }

  def fileUploadError(): Action[AnyContent] = authAction { implicit request =>
