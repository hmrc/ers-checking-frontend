...
object ERSMetrics extends ERSMetrics {
  com.codahale.metrics.SharedMetricRegistries.getOrCreate("default")
 // override def fileProcessingTimer(diff: Long, unit: TimeUnit) = MetricRegistry.MetricBuilder.TIMERS.newMetric().update(diff,unit)//.defaultRegistry.timer("file-processing-time").update(diff, unit)
 // override def besTimer(diff: Long, unit: TimeUnit) = MetricsRegistry.defaultRegistry.timer("bes-processing-time").update(diff, unit)
  ...
  override def dataIteratorTimer(diff: Long, unit: TimeUnit) = com.codahale.metrics.SharedMetricRegistries.getOrCreate("data-iterator-time").timer("data-iterator-time").update(diff, unit)
}
