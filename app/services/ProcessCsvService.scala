...
import uk.gov.hmrc.DataGenerator
import uk.gov.hmrc.CsvValidator.validateCSVFile
...
class ProcessCsvService @Inject()(appConfig: ApplicationConfig,
  ...
  def processFiles(callback: Option[UpscanCsvFilesCallbackList],
                   scheme: String,
                   source: String => Source[Either[Throwable, List[ByteString]], _])(
    ...
    callback.get.files map { file =>
      ...
      val validatorFuture: Future[Either[Throwable, DataValidator]] = Source(List(successUpload.name))
        ...
        .via(eitherFromFunction(DataGenerator.getSheetCsv(appConfig.csopV5Enabled, _, scheme)))
        .via(eitherFromFunction(DataGenerator.identifyAndDefineSheetCsv))
        .via(eitherFromFunction(DataGenerator.setValidatorCsv(appConfig.csopV5Enabled, _)))
        .runWith(Sink.head)
          ...
          (validator: DataValidator) => {
            val futureListOfErrors: Future[Seq[Either[Throwable, RowValidationResults]]] =
              source(successUpload.downloadUrl)
                .via(eitherFromFunction(validateCSVFile(_, successUpload.name, validator)))
  ...
  def getRowsWithNumbers(listOfErrors: Seq[Either[Throwable, RowValidationResults]], name: String)(
    implicit messages: Messages): Either[Throwable, Seq[List[ValidationError]]] = {
