...
import views.html.html_error_report
...
class HtmlReportControllerTest extends UnitSpec with GuiceOneAppPerSuite with ErsTestHelper with Injecting {
  ...
  "html Error Report Page GET" should {
    ...
    "gives a call to showHtmlErrorReportPage if user is authenticated" in {
      lazy val controllerUnderTest = new HtmlReportController(mockAuthAction, mcc, mockErsUtil, mockAppConfig, view)
  ...
  def buildFakeHtmlReportController(fetchAllMapVal: String = "e"): HtmlReportController = {
    ...
    new HtmlReportController(mockAuthAction, mcc, new TestERSUtil, mockAppConfig, view) {
      mockAnyContentAction
    }
  }
