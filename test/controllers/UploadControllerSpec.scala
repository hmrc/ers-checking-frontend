...
class UploadControllerSpec extends TestKit(ActorSystem("UploadControllerTest")) with AnyWordSpecLike with Matchers
  ...
  def buildFakeUploadControllerCsv(uploadRes: Boolean = true,
    ...
    new UploadController(mockAuthAction, mockProcessODSService, mockProcessCsvService, mockSessionCacheService, mcc, globalErrorView) {
      ...
      when(mockSessionCacheService.cache(refEq(ersUtil.FORMAT_ERROR_CACHE), anyString())(any(), any()))
        .thenReturn(if (formatRes) Future.successful(null) else Future.failed(new Exception))
      when(mockSessionCacheService.cache(refEq(ersUtil.FORMAT_ERROR_CACHE_PARAMS), any[Seq[String]])(any(), any()))
        .thenReturn(if (formatRes) Future.successful(null) else Future.failed(new Exception))
      when(mockSessionCacheService.cache(refEq(ersUtil.FORMAT_ERROR_EXTENDED_CACHE), any[Boolean])(any(), any()))
        .thenReturn(if (formatRes) Future.successful(null) else Future.failed(new Exception))

      when(mockSessionCacheService.fetch[UpscanCsvFilesCallbackList](any())(any(), any()))
