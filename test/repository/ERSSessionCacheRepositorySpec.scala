...
class ERSSessionCacheRepositorySpec
    extends PlaySpec with GuiceOneServerPerSuite with BeforeAndAfterEach with MockitoSugar with MongoSupport {
  ...
  def generateTestCacheItem(id: String = "id", data: JsObject = JsObject(Seq("" -> JsString("")))): CacheItem =
    CacheItem(
      id = id,
      data = data,
  ...
  "ERSSessionCacheRepository" when {
    ...
    "fetchAndGetEntry" should {
      "return cached data from session" in {
        ...
        val result: UploadedSuccessfully =
          Await.result(sessionCacheRepo.fetchAndGetEntry[UploadedSuccessfully](sessionId), Duration.Inf)
