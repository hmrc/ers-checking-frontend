...
class DataGeneratorSpec extends PlaySpec with OneServerPerSuite with ScalaFutures with MockitoSugar with BeforeAndAfter with HeaderData{
  ...
  "The File Processing Service" must {
    ...
    "Ensure there are no ampersands in the submitted xml" in {
      ...
      val thrown = intercept[ERSFileProcessingException] {dataGeneratorObj.validateSpecialCharacters(inputXml)(messages)}

      thrown.getMessage must be (expectedMessage)
     }

    "validateHeaderRow " in {
      dataGeneratorObj.validateHeaderRow(XMLTestData.otherHeaderSheet1Data, "Other_Grants_V3", "OTHER", "Other_Grants_V3.csv")(messages) must be (4)
      val result = Try(dataGeneratorObj.validateHeaderRow(XMLTestData.otherHeaderSheet1Data, "csopHeaderSheet1Data", "CSOP", "CSOP_OptionsGranted_V3.csv")(messages))
      result.isFailure must be (true)
    }

    "validate CSOP_OptionsGranted_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(csopHeaderSheet1Data, "CSOP_OptionsGranted_V3", "CSOP", "CSOP_OptionsGranted_V3.csv")(messages) must be (9)
    }
    "validate CSOP_OptionsRCL_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(csopHeaderSheet2Data, "CSOP_OptionsRCL_V3", "CSOP", "CSOP_OptionsRCL_V3.csv")(messages) must be (9)
    }
    "validate CSOP_OptionsExercised_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(csopHeaderSheet3Data, "CSOP_OptionsExercised_V3", "CSOP", "CSOP_OptionsExercised_V3.csv")(messages) must be (20)
    }

    "validate SIP_Awards_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(sipHeaderSheet1Data, "SIP_Awards_V3", "SIP", "SIP_Awards_V3.csv")(messages) must be (17)
    }
    "validate SIP_Out_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(sipHeaderSheet2Data, "SIP_Out_V3", "SIP", "SIP_Out_V3.csv")(messages) must be (17)
    }

    "validate EMI40_Adjustments_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(emiHeaderSheet1Data, "EMI40_Adjustments_V3", "EMI", "EMI40_Adjustments_V3.csv")(messages) must be (14)
    }
    "validate EMI40_Replaced_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(emiHeaderSheet2Data, "EMI40_Replaced_V3", "EMI", "EMI40_Replaced_V3.csv")(messages) must be (17)
    }
    "validate EMI40_RLC_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(emiHeaderSheet3Data, "EMI40_RLC_V3", "EMI", "EMI40_RLC_V3.csv")(messages) must be (12)
    }
    "validate EMI40_NonTaxable_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(emiHeaderSheet4Data, "EMI40_NonTaxable_V3", "EMI", "EMI40_NonTaxable_V3.csv")(messages) must be (15)
    }
    "validate EMI40_Taxable_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(emiHeaderSheet5Data, "EMI40_Taxable_V3", "EMI", "EMI40_Taxable_V3.csv")(messages) must be (20)
    }

    "validate Other_Grants_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet1Data, "Other_Grants_V3", "OTHER", "Other_Grants_V3.csv")(messages) must be (4)
    }
    "validate Other_Options_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet2Data, "Other_Options_V3", "OTHER", "Other_Options_V3.csv")(messages) must be (42)
    }
    "validate Other_Acquisition_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet3Data, "Other_Acquisition_V3", "OTHER", "Other_Acquisition_V3.csv")(messages) must be (40)
    }
    "validate Other_RestrictedSecurities_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet4Data, "Other_RestrictedSecurities_V3", "OTHER", "Other_RestrictedSecurities_V3.csv")(messages) must be (20)
    }
    "validate Other_OtherBenefits_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet5Data, "Other_OtherBenefits_V3", "OTHER", "Other_OtherBenefits_V3.csv")(messages) must be (13)
    }
    "validate Other_Convertible_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet6Data, "Other_Convertible_V3", "OTHER", "Other_Convertible_V3.csv")(messages) must be (15)
    }
    "validate Other_Notional_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet7Data, "Other_Notional_V3", "OTHER", "Other_Notional_V3.csv")(messages) must be (13)
    }
    "validate Other_Enhancement_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet8Data, "Other_Enhancement_V3", "OTHER", "Other_Enhancement_V3.csv")(messages) must be (14)
    }
    "validate Other_Sold_V3 headerRow as valid" in {
      dataGeneratorObj.validateHeaderRow(otherHeaderSheet9Data, "Other_Sold_V3", "OTHER", "Other_Sold_V3.csv")(messages) must be (14)
    }
    ...
    "get an exception if ods file has less than 9 rows and doesn't have header data" in {
      ...
      val result = intercept[ERSFileProcessingException] {
        dataGenObj.getErrors(XMLTestData.getInvalidCSOPWithoutHeaders,"1","CSOP.ods")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
      }
    ...
    "get an exception if ods file has more than 1 sheet but 1 of the sheets has less than 9 rows and doesn't have header data" in {
      object dataGenObj extends DataGenerator
      val result = intercept[ERSFileProcessingException] {
        dataGenObj.getErrors(XMLTestData.getInvalidCSOPWith2Sheets1WithoutHeaders,"1","CSOP.ods")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
      }
      result.message mustBe messages("ers.exceptions.dataParser.incorrectHeader", "CSOP_OptionsGranted_V3", "CSOP.ods")
    }
    ...
    "get an exception if ods file doesn't contain any data" in {
      object dataGenObj extends DataGenerator
      val result = intercept[ERSFileProcessingException] {
        dataGenObj.getErrors(XMLTestData.getCSOPWithoutData,"1","CSOP.ods")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
      }
    ...
    "get no errors for EMI" in {
      object dataGenObj extends DataGenerator
      val result = dataGenObj.getErrors(XMLTestData.getEMIAdjustmentsTemplate,"2","")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
      result.foreach(_.errors.size mustBe 0)
    }

    "collect errors in the first sheet of EMI" in {
      object dataGenObj extends DataGenerator
      val result = dataGenObj.getErrors(XMLTestData.getInvalidEMIAdjustmentsTemplate,"2","")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
      result(0).errors.size mustBe 1
    }

    "collect errors in second sheet of EMI" in {
      object dataGenObj extends DataGenerator
      val result = dataGenObj.getErrors(XMLTestData.getEMIAdjustmentsTemplate ++ XMLTestData.getInvalidEMIReplacedTemplate,"2","")(Fixtures.buildFakeUser,hc = HeaderCarrier(),Fixtures.buildFakeRequestWithSessionId("GET"), messages)
