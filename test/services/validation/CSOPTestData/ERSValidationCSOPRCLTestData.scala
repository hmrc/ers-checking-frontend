...
trait ERSValidationCSOPRCLTestData {
  ...
  def getExpectedResults: List[Option[List[ValidationErrorData]]] = {
    val expectedResults = List(
      ...
      Some(List(ValidationErrorData("error.3", "003", "Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)"))),
      None,
      Some(List(ValidationErrorData("MANDATORY", "018", "Enter a first name (must be less than 36 characters and can only have letters, numbers, hyphens or apostrophes)"))),
      ...
      None,
      Some(List(ValidationErrorData("MANDATORY", "018", "Enter a last name (must be less than 36 characters and can only have letters, numbers, hyphens or apostrophes)"))),
