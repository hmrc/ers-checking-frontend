...
trait ERSValidationEMIAdjustmentsTestData {
  ...
  def getExpectedResults: List[Option[List[ValidationErrorData]]] = {
    val expectedResults = List(
      ...
      None,
      Some(List(
        ValidationErrorData("error.4","004","Enter '1', '2', '3', '4', '5', '6', '7' or '8'."),
      ...
      Some(List(
        ...
        ValidationErrorData("error.6","006","This entry is larger than the maximum number value allowed."),
        ValidationErrorData("error.7","007","This entry must be a whole number."),
        ValidationErrorData("error.8","008","This entry must be either a positive number or a zero.")
      )),
      Some(List(ValidationErrorData("error.7","007","This entry must be a whole number."))),
      None,
      Some(List(ValidationErrorData("error.9","009","Enter a date that matches the yyyy-mm-dd pattern."))),
      None,
      None,
      Some(List(ValidationErrorData("error.10","010","Enter a first name (must be less than 36 characters)."))),
      None,
      Some(List(ValidationErrorData("error.11","011","Must be less than 36 characters."))),
      None,
      Some(List(ValidationErrorData("error.12","012","Enter a last name (must be less than 36 characters)."))),
      None,
      Some(List(ValidationErrorData("error.13","013","The National Insurance number must be 2 letters followed by 6 number digits, with an optional final letter."))),
      None,
      Some(List(ValidationErrorData("error.14","014","PAYE reference must be a 3 digit number followed by a forward slash and up to 10 more characters."))),
      None,
      Some(List(ValidationErrorData("error.15","015","Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."))),
      Some(List(
        ValidationErrorData("error.15","015","Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."),
      ...
      Some(List(
        ValidationErrorData("error.18","018","Must be a number with 2 digits after the decimal point (and no more than 11 digits in front of it)."),
