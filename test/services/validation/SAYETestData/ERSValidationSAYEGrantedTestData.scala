...
trait ERSValidationSAYEGrantedTestData {
  ...
  def getExpectedResults : List[Option[List[ValidationErrorData]]] = List(
    ...
    Some(List(ValidationErrorData("error.3", "003", "This entry must be a number made up of digits."))),
    //C
    None,
    ...
    Some(List(
      ValidationErrorData("error.4", "004", "Must be a number with 2 digits after the decimal point (and no more than 11 digits in front of it)."),
      ValidationErrorData("error.5", "005", "This entry is larger than the maximum number value allowed."),
      ValidationErrorData("error.6", "006", "This entry must be a number made up of digits."))),
    //D
    None,
    Some(List(ValidationErrorData("error.7", "007", "Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."))),
    Some(List(
      ValidationErrorData("error.7", "007", "Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."),
    ...
    Some(List(ValidationErrorData("MANDATORY", "100", "'6. Are the shares listed on a recognised stock exchange? (yes/no)' must have an entry."))),
    //G
    None,
