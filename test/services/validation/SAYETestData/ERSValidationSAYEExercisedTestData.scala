...
trait ERSValidationSAYEExercisedTestData {
  ...
  def getTestData : List[Cell] = List(
    ...
    Cell("K", rowNumber, "obc12345678901"),
    //L
  ...
  def getExpectedResults: List[Option[List[ValidationErrorData]]] = List(
    ...
    Some(List(ValidationErrorData("MANDATORY", "100", "'7. Date of grant (yyyy-mm-dd)' must have an entry."))),
    //H
    None,
    ...
    Some(List(ValidationErrorData("MANDATORY", "100", "'9. Were the shares subject to the option listed on a recognised stock exchange? (yes/no) If yes go to question 12 If no go to question 10' must have an entry."))),
    //J
    None,
    ...
    Some(List(
      ValidationErrorData("error.14", "014","Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."),
      ValidationErrorData("error.15", "015","This entry must be a number made up of digits."),
      ValidationErrorData("error.16", "016", "This entry is larger than the maximum number value allowed.")
    )),
    Some(List(ValidationErrorData("error.16", "016", "This entry is larger than the maximum number value allowed."))),
    //M
    None,
    ...
    Some(List(
      ValidationErrorData("error.20", "020","Must be a number with 4 digits after the decimal point (and no more than 13 digits in front of it)."),
    ...
    //O
    None,
    Some(List(ValidationErrorData("error.23", "023", "Enter 'yes' or 'no'."))),
    Some(List(ValidationErrorData("MANDATORY", "100", "'15. Does the exercise qualify for tax relief? (yes/no)' must have an entry."))),
    //P
    None,
    Some(List(ValidationErrorData("error.24", "024", "Enter 'yes' or 'no'."))),
    Some(List(ValidationErrorData("MANDATORY", "100", "'16. Were all shares resulting from the exercise sold? (yes/no). Answer yes if they were either sold on the same day as the exercise in connection with the exercise or sale instructions were given for all shares to be sold on exercise' must have an entry.")))
  )
}
