...
trait ERSValidationOTHERConvertibleTestData {
  ...
  def getExpectedResults: List[Option[List[ValidationErrorData]]] = {
    val expectedResults = List(
      ...
      Some(List(ValidationErrorData("MANDATORY","100","'13. Has a National Insurance Contribution election or agreement been operated (yes/no)' must have an entry."))),
      //N
      None,
      Some(List(ValidationErrorData("error.20","020","This entry must be 'yes' or 'no'."))),
      Some(List(ValidationErrorData("MANDATORY","100","'14. Was PAYE operated? (yes/no)' must have an entry."))),
      //O
      None,
      Some(List(ValidationErrorData("error.21","021","This entry must be 'yes' or 'no'."))),
      Some(List(ValidationErrorData("MANDATORY","100","'15. Was any adjustment made for amounts subject to apportionment for residence or duties outside the UK? (yes/no)' must have an entry.")))
    )
    expectedResults
  }
